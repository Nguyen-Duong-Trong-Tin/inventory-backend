...
export class LotsService extends BaseCrudService<Lot> {
  ...
  async createLot({ body }: { body: CreateLotBodyDto }) {
      const { lotNumber, manufactureDate, expiryDate, productId, warehouseReceiptId } = body;
  ...
  async updateLot({
      ...
      const newLot = await this.findOneAndUpdate({
        filter: { _id: id },
        update: { lotNumber, manufactureDate, expiryDate, productId, warehouseReceiptId  },
      });
      if (!newLot) {
  ...
  async findLots({ query }: { query: FindLotsQueryDto }) {
    ...
    if (filter) {
      const { lotNumber, manufactureDate, expiryDate, productId, warehouseReceiptId, sortBy, sortOrder } = filter;
